...
public class AdminUsersTest extends BaseClass {
	...
	@Test(priority = 1, description = "Trying to add a new user")
	public void verifyWheteherUserIsAbleToAddANewUser() throws IOException {
		...
		admin.newBtnClick().enterUsernameOnUsernameField(userNameVal).enterPasswordOnPasswordField(passwordVal)
				.selectDrpDwn(typeVal).saveBtnClick();
		String actual = admin.AlertDisplayed();
		Assert.assertTrue(actual.contains("User Created Successfully"), Constant.UserNotAddedError);

	}

	@Test(priority = 1, description = "Trying to search a  user")
	public void verifyWhetherUserIsAbleToSearchAUser() throws IOException {
		...
		String typeVal = ExcelUtility.getStringData(0, 2, "AdminUserPage");
		admin.searchBtnClick().enterUsernameOnSearchUsernameField(userNameVal).searchDrpDwn(typeVal)
				.searchUserBtnClick();
	...
	@Test(priority = 1, description = "Trying to reset the page")
	public void verifyWhetherUserIsAbleToResetThePage() throws IOException {
		String userNameValue = ExcelUtility.getStringData(0, 0, "LoginPage");
		String passwordValue = ExcelUtility.getStringData(0, 1, "LoginPage");
		LoginPage login = new LoginPage(driver);
		login.enterUsernameOnUsernameField(userNameValue).enterPasswordOnPasswordField(passwordValue);
